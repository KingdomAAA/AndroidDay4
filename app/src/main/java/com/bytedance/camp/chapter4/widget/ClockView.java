...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        float secondDegree = second * 360  / 60 - 90 ;
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        float endSecondX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(secondDegree)));
        float endSecondY = (float) (centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(secondDegree)));
        canvas.drawLine(centerX,centerY,endSecondX,endSecondY,needlePaint);

        float minuteDegree = minute  * 360 / 60 +  second * 6 / 60 - 90;
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        float endMinuteX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(minuteDegree)));
        float endMinuteY = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(minuteDegree)));
        canvas.drawLine(centerX,centerY,endMinuteX,endMinuteY,needlePaint);


        float hourDegree = (float) (hour * 360 / 12 + minute * 30 / 60 + second * 0.1 / 60 - 90);
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        float endHourX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(hourDegree)));
        float endHourY = (float) (centerY + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(hourDegree)));
        canvas.drawLine(centerX,centerY,endHourX,endHourY,needlePaint);
    }

    private void drawTimeNumbers(Canvas canvas) {
        // TODO 绘制表盘时间数字（可选）
        float Radius = radius - 100;
        float shortX = (float) (Radius * Math.sin(Math.toRadians(30)));
        float longX = (float) (Radius * Math.cos(Math.toRadians(30)));
        Paint.FontMetrics fontMetrics = numberPaint.getFontMetrics();
        float top = fontMetrics.top;
        float bottom = fontMetrics.bottom;
        int baseLineY = (int) (centerY + ((bottom-top)/2-bottom));
        canvas.drawText("12",centerX,baseLineY-Radius,numberPaint);
        canvas.drawText("11",centerX-shortX,baseLineY-longX,numberPaint);
        canvas.drawText("10",centerX-longX,baseLineY-shortX,numberPaint);
        canvas.drawText("09",centerX-Radius,baseLineY,numberPaint);
        canvas.drawText("08",centerX-longX,baseLineY+shortX,numberPaint);
        canvas.drawText("07",centerX-shortX,baseLineY+longX,numberPaint);
        canvas.drawText("06",centerX,baseLineY+Radius,numberPaint);
        canvas.drawText("05",centerX+shortX,baseLineY+longX,numberPaint);
        canvas.drawText("04",centerX+longX,baseLineY+shortX,numberPaint);
        canvas.drawText("03",centerX+Radius,baseLineY,numberPaint);
        canvas.drawText("02",centerX+longX,baseLineY-shortX,numberPaint);
        canvas.drawText("01",centerX+shortX,baseLineY-longX,numberPaint);
    }
